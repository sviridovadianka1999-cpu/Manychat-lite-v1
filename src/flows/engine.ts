...
import { sendCommentReply, sendDm } from "@/src/meta/service";
...
export async function startExecution(flowId: string, definition: FlowDefinition, contactId: string | null, context: ExecutionContext, triggerType: string) {
  ...
  if (next) await runNode(executionId, flowId, definition, next, contactId, context);
  return executionId;
}
  ...
  for (const step of steps.rows) {
    const locked = await query(
      `UPDATE flow_execution_steps
        ...
        WHERE execution_id=$1 AND node_id=$2 AND status='waiting'`,
      [step.execution_id, step.node_id]
    );
    if (locked.rowCount !== 1) continue;

    await query(`UPDATE flow_executions SET status='running' WHERE id=$1 AND status='waiting'`, [step.execution_id]);
    const next = getNextNode(step.definition_json, step.node_id);
    if (next) await runNode(step.execution_id, step.flow_id, step.definition_json, next, step.contact_id, step.context_json);
  }
}

async function runNode(executionId: string, flowId: string, definition: FlowDefinition, nodeId: string, contactId: string | null, context: ExecutionContext) {
  ...
  try {
    ...
    if (node.type === "condition") {
      const ok = await evalCondition(configObject(node.config), contactId, context, executionId, flowId);
    ...
    if (node.type === "action") {
      const result = await execAction(configObject(node.config), contactId, context, executionId);
      const stepStatus = result.waiting ? "waiting" : "done";
      ...
      if (result.waiting) {
        await query(
          ...
          [executionId, nodeId, String(result.waitSeconds ?? 5)]
        );
...
async function evalCondition(config: Record<string, unknown>, contactId: string | null, context: ExecutionContext, executionId: string, flowId: string) {
  ...
  if (type === "flow_not_completed_before" && contactId) {
    const r = await query<{ count: string }>(
      `SELECT count(*) FROM flow_executions WHERE contact_id=$1 AND flow_id=$2 AND status='completed'`,
      [contactId, flowId]
    );
...
async function execAction(config: Record<string, unknown>, contactId: string | null, context: ExecutionContext, executionId: string) {
  ...
  if ((type === "send_comment_reply" || type === "send_private_reply") && context.commentId) {
    const result = await sendCommentReply(context.commentId, String(config.text ?? ""));
    await saveOutgoing(contactId, "send_comment_reply", config, result);
    return { stopped: false, waiting: false };
  }
  ...
  if (type === "add_tag" && contactId) {
    await addTag(contactId, String(config.tag ?? ""));
    return { stopped: false, waiting: false };
  }
  if (type === "remove_tag" && contactId) {
    await removeTag(contactId, String(config.tag ?? ""));
    return { stopped: false, waiting: false };
  }
  if (type === "set_custom_field" && contactId) {
    await setCustomField(contactId, String(config.field_key ?? ""), String(config.field_value ?? ""));
    return { stopped: false, waiting: false };
  }
  if (type === "wait") {
    return { stopped: false, waiting: true, waitSeconds: Number(config.seconds ?? 5) };
  }
  if (type === "stop_flow") {
    await query(`UPDATE flow_executions SET status='stopped', finished_at=now() WHERE id=$1`, [executionId]);
    return { stopped: true, waiting: false };
  }
...
function configObject(config: FlowDefinition["nodes"][number]["config"]) {
